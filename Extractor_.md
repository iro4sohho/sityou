...
※YouTubeの仕様上、必ずしも投稿されたすべてのチャットデータを抽出できるとは限りません。<br>（pytchatブラウザは一般的なブラウザが取得するのと同じチャットデータを取得していますが、<br>一般的なブラウザで表示されるアーカイブのチャット自体、ライブ配信時に表示されていたチャットデータの一部が保存されていない場合があります。<br>（「すべてのチャット」に切り替えていても同様）<br>ライブ当時のチャットの流れが速い部分ではアーカイブ後にYouTubeが返すチャットデータに欠落が生じている場合があり、pytchatのextractorで取得できるデータはこれに準じます。<br>
<br>
...
このため、callbackから渡されるデータは進捗状況を把握するためだけに利用することを推奨します。<br>
